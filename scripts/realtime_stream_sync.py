...
class GStreamerAudio:
    def __init__(self):
        pipeline_str = (
            ...
            "udpsink host=127.0.0.1 port=5001 sync=true"
        )
        self.process = subprocess.Popen(
            f"gst-launch-1.0 {pipeline_str}",
            stdin=subprocess.PIPE,
    ...
    def send_audio(self, audio_data):
        try:
            ...
            self.process.stdin.flush()
            print(f"✅ Pushed audio chunk, {len(audio_data)} samples")
        except Exception as e:
...
class Avatar:
    ...
    def process_frames(self, res_frame_queue, video_len, gst_video_pipeline, gst_audio_pipeline, skip_save_images, debug=False):
        ...
        while True:
            if self.idx >= video_len - 1:
                break
            try:
                batch = res_frame_queue.get(block=True, timeout=1)
            except queue.Empty:
                continue

            if not batch or not isinstance(batch, tuple):
                continue
            ...
            for i, res_frame in enumerate(frames):
                ...
                if combine_frame is not None:
                    resized = cv2.resize(combine_frame, (640, 480))
                    prepared_frames.append(resized)
                self.idx += 1

            if prepared_frames and audio_chunk is not None:
                for frame in prepared_frames:
                    gst_video_pipeline.send_frame(frame)  
                gst_audio_pipeline.send_audio(audio_chunk)  
    ...
    def inference(self, audio_path, out_vid_name, fps, skip_save_images):
        ...
        gst_video_pipeline = GStreamerPipeline()
        ...
        self.idx = 0
        process_thread = threading.Thread(target=self.process_frames, args=(res_frame_queue, len(whisper_chunks), gst_video_pipeline, gst_audio_pipeline, skip_save_images))
        ...
        for i, (whisper_batch, latent_batch) in enumerate(tqdm(gen, total=int(np.ceil(float(video_num) / self.batch_size)))):
            ...
            frame_count += len(recon)
            res_frame_queue.put((recon, audio_chunks[i]))
        
        process_thread.join()
        gst_video_pipeline.stop()
        gst_audio_pipeline.stop()
